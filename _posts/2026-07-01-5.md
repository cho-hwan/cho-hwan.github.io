...
2. [Youtube Algorithm Course - Graph Theory from a Google Engineer](https://www.youtube.com/watch?v=09_LlHjoEiY&t=9751s)
...
A graph is said to be $d\text{-regular}$ if all nodes are of $\text{degree-}d$. where degree is defined as the number of edges incident on each vertex. The below graph has diameter 2 but is not $d\text{-regular}$ since some nodes are of degree 2 and some are of degree 3.
