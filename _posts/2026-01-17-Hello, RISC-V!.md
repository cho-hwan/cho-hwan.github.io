...
하지만 2003년경, 발열과 전력 소모, 그리고 ILP(명령어 수준Basic 병렬성)가 한계에 부딪히며 이러한 흐름은 정체되었습니다. 제조사들은 클럭 주파수를 높이는 대신 멀티코어로 선회했고, 이때부터 성능을 제대로 뽑아내기 위해 병렬 프로그래밍과 컴퓨터 구조에 대한 깊은 이해는 필수 역량이 되었습니다.
...
또한, Inference Performance에 관심이 많은 저에게 특수 목적 가속기에서 RISC-V가 보여주는 성장세가 매력적으로 다가왔습니다. 물론 산업계 전체로 보면 RISC-V의 점유율은 여전히 저조합니다. 리스크가 굉장히 많이 존재하는 것도 사실이고요.  RISC-V를 사용했을때 설계 검증 및 툴체인 안정화에 긴 리드타임이 걸리는 것도 부담이고, 독자적으로 기술 개발을 해야하는 것도 부담이고, 소프트웨어 생태계를 구축해야하는 것도 부담이고.. 이 외에도 굉장히 많은 부담을 안겨줍니다. 그렇지만 이는 저에게는 좋은 기회라고 생각합니다. ARM이 로얄티를 파격세일 하지 않는 이상, 다양한 도메인에 fit하게 resource를 최적화해주는 가속기는 도메인마다 다양한 방식으로 발전할 것 같았기 때문이죠. 모르겠고 아무튼 RISC-V 화이팅!!!
...
지금까지는 메모리 블록을 캐시에 넣을 때 각 블록이 캐시의 딱 한 곳에만 들어갈 수 있는 단순한 `Direct Mapped` 방법을 사용했습니다. 하지만 실제로는 `TLB`  블록을 배치하는 다양한 방식이 존재하는데, 그 중 중요한 방식이 `Set Associative(집합 연관)`방식입니다.
...
>Virtual Machine이 Memory Hierarchy에 Mapping되지는 않기에 Virtual Machine를 어떤 카테고리로 분류해야할까 많은 고민을 했습니다. 그랬지만 마땅히 좋은 위치를 찾지 못해 [Computer Organization and Design RISC-V](https://www.amazon.com/Computer-Organization-Design-RISC-V-Architecture/dp/0128203315) 의 분류 카테고리를 그대로 따랐습니다. 아무래도 Virtual Machine의 핵심 중 한가지가 Address Relocation이기에 이러한 분류는 꽤나 괜찮은 것 같습니다.
...
VM은 사실 꽤나 넓은 영역을 지니고있습니다.Java VM같이 표준 소프트웨어 인터페이스를 제공하는 모든 에뮬레이션 또한  VM으로 취급되는데요, 블로그에서는 ISA수준에서 완전한 시스템 계층 환경을 제공하는 System Virtual Machine을 다루겠습니다. 
...
{: .prompt-info}
>요즘은 가상화 기술이 많이 보편화되어 Virtual Box, EC2 와 같은 VM을 다들 많이 접해봤을겁니다. VM을 사용하면 완벽한 소프트웨어 스택을 실행할 수 있는 추상화를 제공받을 수 있고, 하드웨어와 무관하게 실행 환경을 어디든 이식할 수 있다는 장점으로 인해 VM을 사용하는 것 같습니다.

![privilege mode](/assets/post/2026-01-17-Hello,-RISC-V!/privilege_mode.png)

앞서 살펴본 **[RISC-V Privilege Levels]** 의 핵심은 가상화 환경에서도 그대로 적용됩니다. VMM은 시스템 전체를 통제해야 하므로, 일반적인 Guest OS나 애플리케이션보다 반드시 높은 권한을 가져야 합니다.

따라서 시스템은 최소한 2개 이상의 프로세스 모드(Supervisor Mode, User Mode)를 지원해야 하며, VMM은 이 중 가장 높은 권한인 Supervisor(또는 그 이상의 확장 모드)에서 동작합니다. 반면, Guest VM 내에서 실행되는 OS와 Application들은 VMM보다 낮은 권한 계층에서 실행되며, 자원을 직접 제어하려는Privileged Instruction를 내릴 경우 Trap이 발생하여 VMM에 의해 통제됩니다.



{: .prompt-tip}
>RISC-V는 Virtualizable하다. 이는 VM을 고려한 ISA 설계를 통한 결과이며, 이로 인해 RISC-V는 Trap에 걸릴 법 한 명령어를 모두 찾아내고, 이 명령어들이 Guest OS에 의해 실행되게끔 하는 과정에서 낮은 복잡성을 가지기 때문에 VMM이 명령어를 처리할 때 발생하는 오버헤드가 다른 OS(x86-64, ARMv7, MIPS 등)에 비해 낮습니다.

{: .prompt-tip}
> System Virtual Machine과 이후에 다룰 Virtual Memory는 추상화라는 개념을 공유한다는 점에서 비슷해 보이지만 전혀 관계 없는 개념입니다. 두 개념을 혼동하거나 비슷한 거라고 착각하면 안됩니다.

### Virtual Memory

Virtual Memory는 Storage 데이터를 RAM에서 Cash 로서 동작할 수 있도록 하는 기술을 의미합니다. 

Virtual Memory의 탄생 과정을 알면 참 또 재밌는데요, Virtual Machine과는 다르게 일반 사용자가 추상화 컨너편을 바라볼 기회가 거의 없다시피 하기 때문에 책에서 제공하는 Virtual Memory의 배경 설명이 저에게는 큰 도움이 되었습니다.

{: .prompt-tip}
>근데 정말 초초초초창기 Virtual Memory는 시분할 시스템의 일부로 다수의 프로그램이 컴퓨터를 공유할 수 있게 하기 위해서 개발되었다고 합니다. 근데 제가 시분할 시스템에 대한 이해가 전혀 없기에 패스하겠습니다.

역사적으로 Virtual Memory를 사용하게 된 주요 동기는 두 가지 라고 합니다. 첫째는 여러 프로그램들이 효과적이고 안전하게 메모리를 공유할 수 있게 하기 위함입니다. 클라우트 컴퓨팅을 위한 여러 가상 머신의 메모리 요구가 이런 경우이죠. 둘째는 작고 제한된 크기의 메인 메모리에서 프로그래밍하는 제약을 제거하기 위함입니다. 이 기법이 탄생한 지 50년이 지난 지금은 첫 번째 이유로 널리 사용되고 있다고 하네요.

아무튼 차차하고, 다수의 VM이 같은 메모리를 사용할 수 있게 하려면, 모든 각기 다른 VM에서 작동하는 프로그램이 반드시 자신에게 할당된 메인 메모리 부분만 읽고 쓸 수 있게 하여 각 VM을 다른 VM으로부터 보호할 수 있어야 합니다.

프로그램을 컴파일할 때는 VM이 다른 어떤 VM과 메모리를 공유할지 알 수 없습니다. 메모리를 공유하는 VM들은 로직이나 동작은 일정하지만 VMM이 개입하면서 실제 데이터가 담기는 물리적 위치(Physical Memory)는 동적으로 변화합니다. 이러한 동적 상호작용 과정이 있어야 서로 다른 VM에서 실행되는 프로그램의 Physical Address를 다른 VM이 간섭하지 못하게 막아줍니다.

![overlay](/assets/post/2026-01-17-Hello,-RISC-V!/overlay.webp)

그리고 사용자 프로그램이 물리 메모리보다 커질 수 있도록 지원하기 위함도 다뤄야합니다. 과거에는 프로그램 크기가 메인 메모리를 초과하면 오버레이(Overlay)라는 방식을 통해 할당 메모리를 조절하는 작업이 전적으로 프로그래머의 몫이었습니다. 프로그래머는 프로그램을 작은 모듈 단위로 쪼개고, 동시에 실행될 필요가 없는 상호 배타적인 모듈을 직접 찾아내야 했습니다. 지금은 그렇지 않지만 말이죠~


#### Page Table and TLB

![simple_architecture_virtual_memory](/assets/post/2026-01-17-Hello,-RISC-V!/simple_architecture_virtual_memory.png)
`Virutal Memory`에 대한 시각화입니다. 모든 프로세스들이 Virtual Address를 사용하기 때문에 프로세스들은 자신만의 Page Table을 가지고 있습니다. 그렇기에 운영체제가 서로 다른 프로세스들간의 Virtual Address 공간들이 충돌하지 않도록 Physical Memory를 할당하고 Page Table을 갱신해주죠.

`Page Fault`로 인해 운영체제가 제어권을 갖게 되면 계층은 Storage(or Flash Memory)내에 존재하는 해당 프로세스의 `Swap Space`에서 데이터를 가져오게 됩니다. 그리고 `Page Table`의 공간이 모자를 경우에도 이전에 다뤘던 `(유사)LRU`방식을 사용해 엔트리를 교체하는데 이 때에도 Swap Space에 저장됩니다.

디스크의 경우 전송시간이 접근시간보다 짧기 때문에 쓰기에서는 Write Back 전략을 취하며, `dirty bit`가 사용됩니다. 그리고 TLB가 없으면 메모리 접근은 2번에 거쳐 Sequantial하게 진행됩니다. TLB는 메모리 접근을 Parallel하게 가져갈 수 있도록 도와주죠.

`TLB(Translation Lookaside Buffer)`는 RAM에 할당되어 운영체제가 관리하는 데이터 구조가 아니라, CPU 패키지 내부 `MMU(Memory Management Unit)`에 통합된 하드웨어 컴포넌트입니다. 이러한 이유로 엔트리를 많이 갖지 못해 `TLB Miss`가 `Page Fault`보다 훨씬 빈번하게 발생합니다. 

TLB의 보편적인 값은 아래와 같습니다.

- **TLB Size:** 16 ~ 512 엔트리
- **Block Size:** 1 ~ 2 Page Table 엔트리
- **Hit Time:** 0.5 ~ 1 Clock Cycle
- **Miss Penalty:** 10 ~ 100 Clock Cycle
- **Miss Rate:** 0.01 ~ 1% 

TLB는 캐시 메커니즘을 따르기에 다양한 Set Associative를 적용할 수 있으나, 일반 캐시 대비 용량이 압도적으로 작다는 구조적 차이가 있습니다. 또한, `Fully Associative`를 도입하면 주소 검색 속도는 향상되지만, LRU와 같은 복잡한 교체 알고리즘을 하드웨어로 구현하는 데 막대한 리소스와 비용이 소모된다는 Trade off가 존재합니다.

{: .prompt-tip}
>완벽한 `LRU`구현을 위해서는 메모리를 참조할 때마다 자료구조를 갱신해야하기 때문에 비용이 많이 듭니다. 그래서 RISC-V 컴퓨터의 운영체제는 일정 주기로  `reference bit`를 0으로 만들고 후에 참조된 페이지만 다시 1로 만드는 방식을 사용하여 `LRU`를 구현합니다. `reference bit`가 제공되지 않는 Architecture의 운영체제는 다른 방식으로 해당 기능을 구현합니다.
...
그리고 재밌게도 Cache에서 Tag와 Index를 Virtual Address에서 가져오냐, Physical Address에서 가져오냐에 따라 PIPT, VIPT(VIVT, PIVT는 생략)으로 나눌 수 있으며,  구분에 따라  다양한 특성과 문제가 발생합니다.

#### Cache Addressing Schemes: PIPT vs. VIPT

![PIPT](/assets/post/2026-01-17-Hello,-RISC-V!/PIPT.png)

우선 정석적인 방식 `PIPT(Physically Indexed, Physically Tagged)`입니다. 이 방식은 아무래도 TLB 조회 + Cache Index 조회가 직렬적으로(Serial) 수행되어야하므로 다른 방식에 비해 상대적으로 느립니다. 그렇지만 이후에 설명할 VIPT 에서 발생하는 aliasing, homonym 문제가 발생하지 않는다는 장점이 있어 속도보다 용량과 정확성이 중요한 하위레벨 캐시에서 많이 사용됩니다. 실제 루나레이크에서는 L0/L1캐시에서 `VIPT`가, L2/L3 캐시에서는 `PIPT`가 사용되었습니다.
...
#### Mechanisms for Virtual Memory Protection

위에서 다룬 Homonym에 대한 해결방법이 Mechanisms for Virtual Memory Protection의 아주 중요한 예시입니다.

{: .prompt-tip}
>운영체제의 예외 처리기 실행이 시작될 때부터 운영체제가 프로세서의 모든 상태를 저장하는 시간 사이에는 운영체제가 매우 취약한 상태입니다. 예외를 활성화하거나 비활성화하는 기능을 제공함으로써 이 문제를 해결할 수 있습니다.

가상 메모리는 물리 메모리보다 큰 공간을 활용할 수 있게 해주지만, 프로세스가 실제 메모리 용량보다 과도하게 가상 메모리에 접근하면 디스크 I/O 비용으로 인해 성능이 급격히 저하되는 `Thrashing`현상이 발생합니다. `Thrashing`은 발생 시 손실이 매우 크기 때문에 메모리를 늘리는 것이 일반적인 해결책입니다. 하지만 컴퓨터 전공자라면 알고리즘과 자료구조를 재검토하여 Locality를 개선함으로써 Working Set 규모를 줄이는 시도해볼 수 있습니다.

실제로 `Thrashing`보다 빈번하게 발생하는 성능 병목은 `TLB Miss`입니다. `TLB`는 보통 32~64개 정도의 엔트리만 처리할 수 있어, 프로세서가 즉각 접근 가능한 범위가 $64 \times 4 \, \text{KiB} = 0.25 \, \text{MiB}$ 수준에 불과하기 때문입니다. 그렇기에 `Radix Sort`와 같이 광범위한 메모리 접근이 일어나는 경우 높은 `TLB Miss`를 보입니다. 이를 완화하기 위해 RISC-V와 같은 현대 하드웨어는 기본적으로 $4 \, \text{KiB}$ 외에도 $2 \text{MiB}$나 $1 \text{GiB}$의 `Multiple Page`를 지원합니다. 큰 페이지를 사용하면 `TLB Miss` 없이 직접 접근 가능한 메모리 영역이 비약적으로 넓어지기 때문입니다.

그러나 운영체제 차원에서 큰 페이지 사용을 허용하는 것은 또 다른 문제입니다. 수백 개의 프로세스가 돌아가는 서버 환경에서 큰 페이지는 `Memory Fragmentation`을 심화시킬 수 있어 OS 설계자들이 아주 싫어하기 때문입니다. 그렇기에 결국 소프트웨어 수준에서 알고리즘과 자료구조를 최적화하여 `Working Set`을 최소화하는 것이 가장 현실적이고 효과적인 전략이 될 수 있을 것 같습니다.

## Parallel Processor

병렬 처리의 도전적인 과제는 하드웨어가 아닙니다. 문제는 멀티프로세서에서 더 빨리 수행하도록 재작성된 응용 프로그램들의 수가 너무 적다는 것입니다. 멀티프로세서를 사용하여 1개의 태스크를 더 빨리 수행하는 소프트웨어를 작성하는 것이 너무 어렵기 때문이죠.

### The Scaling Limit of Parallel Systems: Amdahl’s Law

이번 챕터에서는 [Computer Organization and Design](https://shop.elsevier.com/books/book-companion/9780128203316)에서 제공하는 예제를 그대로 사용합니다. 해당 예제를 통해 Parallel System에서 겪는 Speed-up challenge에 대해 알아보겠습니다.
#### Speed-up Challenge: Bigger Problem

Bigger Problem을 쉽게 이해하려면 `strong scaling`과 `weak scaling`에 대해 알아야 하는데요. `strong scaling`은 문제의 크기를 고정시킨 상태에서 얻어지는 속도 개선입니다. 문제의 크기 `M`이 메인 메모리 내의 `working set`과 같고 프로세서는 `P`개가 있다고 가정할 때, `strong wcaling`의 프로세서당 메모리는 대략 `M/P`이 되죠.

그러면 예제를 통해 수치적으로 나타내보죠. 아래 문제를 풀어보고 이후의 글을 읽으면 더 도움이 됩니다.

{: .prompt-info}
> **예제 1**: 100개의 프로세서를 가지고 90배의 속도 개선을 얻기로 원하는 누군가가 있다고 합시다. 원 프로그램에서 최대 몇 %까지는 순차적으로 실행되도 좋을까요?

우선 1장에서 다룬 `Amdahl의 법칙`이 사용되니 한번 짚고 갑시다. 이 법칙은 쉽지만 굉장히 중요하니 꼭 알아두시기 바랍니다.

$$ \text{ExT(Execution time) with enhancement} = \frac{\text{ExT affected by enhancement}}{\text{Amount of improvement}} + \text{ExT unaffected by enhancement} $$

이 `Amdahl의 법칙`을 $\text{Overall Speedup}$와 $\text{Execution time before enhancement}$ 의 관계식으로 다시 작성하면 아래와 같이 표현할 수 있습니다.

$$\text{Overall Speedup} = \frac{\text{ExT before enhancement}}{\underbrace{(\text{ExT before enhancement} - \text{ExT affected by enhancement})}_{\text{영향을 받지 않는 부분}} + \underbrace{\frac{\text{ExT affected by enhancement}}{\text{Amount of improvement}}}_{\text{개선되어 줄어든 부분}}}$$

$\text{Execution time before enhancement} = 1$이라고 가정하고, 식을 다시 정리하면 개선될 부분의 시간이 $\text{Execution time before enhancement}$에 대한 비율로 바뀝니다. 거기에 기대하는 $\text{Overall Speedup} = 90$ 을 식에 대입하고 $\text{Amount of Improvement} = 100$을 대입하면 아래와 같이 나타나죠.

$$90 = \frac{1}{(1 - \text{ExT affected by enhancement}) + \frac{\text{ExT affected by enhancement}}{100}}$$

$$\therefore \text{ExT affected by enhancement} \approx 0.999$$

결국 100개의 프로세서를 이용하여 90배의 속도 개선을 얻기 위해서는 전체 프로그램에서 순차적인 부분의 비율이 0.1% 이하가 되어야 합니다. 즉 병렬화가 가능한 부분이 99.9% 이상이여야 가능한 수치인거죠.

 애초에 성능 개선 폭을 너무 높게 잡은건 맞으나, 이는 병렬처리를 통한 성능 개선이 타이트하게 기존 프로그램을 손봐야 가능함을 보여줍니다.


>`weak scaling`은 프로세서의 수에 비례하여 문제 크기를 증가시키는 경우의 속도 개선입니다. 문제의 크기 M이 메인 메모리 내에 `working set`과 같고 프로세서는 `P`개가 있다고 가정할 때, `weak scaling`의 경우 프로세서당 메모리는 대략 `M`이 됩니다.

{: .prompt-info}
> **예제 2**: 어떤 프로그램이 10개의 스칼라 변수의 합을 구하고 10 × 10 2차원 행렬의 합을 구하는 덧셈을 수행한다고 가정하자. 10개의 프로세서를 사용하는 경우에 비해서 40개의 프로세서를 사용할 경우 얼마의 속도 개선을 얻을 수 있을까? 또 행렬의 크기를 20 × 20으로 늘렸을 때의 속도 개선을 구하시오.

덧셈 연산을 수행하는 시간을 `t`라고 하고 성능이 t의 함수라고 가정합시다. 이때 스칼라 변수 10개의 덧셈은 병렬 프로세서를 사용해도 좋아지지 않지만, 100번의 덧셈은 병렬 프로세서를 사용하면 좋아질 수 있습니다. 


$$\begin{aligned} \text{Execution time with enhancement (10 Processor)} &= \frac{100t}{10} + 10t = 20t \\ \text{Execution time with enhancement (40 Processor)} &= \frac{100t}{40} + 10t = 12.5t \\ \\ \implies \text{Speedup} = \frac{20t}{12.5t} &= \mathbf{1.6 \text{배 속도 개선}} \end{aligned}$$


위에서와 같이 10개의 프로세서 대비 40개의 프로세서를 사용하면 1.6배의 속도 개선을 얻을 수 있습니다.


$$\begin{aligned} \text{Execution time with enhancement (10 Processor)} &= \frac{400t}{10} + 10t = 50t \\ \text{Execution time with enhancement (40 Processor)} &= \frac{400t}{40} + 10t = 20t \\ \\ \implies \text{Speedup} = \frac{50t}{20t} &= \mathbf{2.5 \text{배 속도 개선}} \end{aligned}$$


행렬의 크기가 20 × 20으로 증가하면 `2.5배` 속도가 향상되는 것도 볼 수 있습니다. 이 예제를 통해 알 있는 바는 문제의 크기를 증가시키면서 속도를 뽑는 것(weak scaling)이 문제의 크기를 고정시킨 상태에서 속도를 뽑는 것(strong scaling)보다 쉽다는 것입니다.

이론적으로는 문제 크기를 키우면 $sequential$의 비중이 줄어듭니다. 그래서 프로세서를 많이 투입한 만큼 속도가 쭉쭉 올라가는 것처럼 보이죠. 그렇지만 `memory hierarchy`가 너무 커져서 멀티코어 마이크로프로세서의 마지막 단계 캐시에 다 들어가지 않으면, `strong scaling`을 사용하는 경우보다 성능이 나빠질 수 있는 것을 명심해야합니다.

`Bigger Problem`을 다루며 우리는 모든 프로세서에 동일한 부하가 가해진다고 가정했습니다. 그렇지만 실제로는 그렇지 않죠. 코어마다 그리고 `In-cheap Accelerator`가 일반적이게 된 지금, 프로세서마다의 부하는 굉장히 차이가 많이 납니다. 그렇기에 프로세서마다 얼마나 부하가 다른지에 따른 성능 개선 예제도 살펴봅니다. 

#### Speed-up Challenge: Balancing Load

앞의 예제에서 40개의 프로세서를 사용하여 속도 개선을 얻기 위해서 우리는 부하(Load)가 완전히 균형을 이룬(nice Balance) 것으로 가정했습니다. 즉 각 프로세서가 `2.5%`씩 부하를 나누어가졌다고 생각한거죠. 이제 아닌 경우를 다루어볼겁니다.

{: .prompt-info}
>**예제 3:** 한 프로세서의 부하가 2배로 증가하는 경우(5%)와 5배로 증가하는 경우(12.5%)의 속도 개선을 계산하시오. 그리고 나머지 프로세서가 얼마나 잘 활용되고 있는지 서술하시오

한 프로세서가 병렬로 수행될 부분의 $5\%$를 담당하면 $\text{5\%} \times \text{400}$ 즉 20개의 덧셈을 수행하고 나머지 39개의 프로세서가 380개의 덧셈을 나누어 수행하게 됩니다. 그러나 이 연산들은 모두 동시에 수행되므로 가장 늦은 프로세서의 수행시간으로 최종 실행시간이 결정되게 됩니다.

$$\text{Execution time with Enhancement} = \max(\frac{380t}{39}, \frac{20t}{1}) + 10t = 30t$$
성능 개선의 폭이 20.5에서 410t/30t = 14로 감소했습니다. 한 프로세서가 20t동안 쉬지 않고 일을 수행하는 동안 나머지 39개의 프로세서는 380t/39  = 9.7t 시간동안 일하고 나머지 반 이상의 시간을 가만히 있었기 때문이죠.

해당 프로세서의 부하를 더 걸어줄수록 속도 개선을 더 떨어집니다. 당연한겨죠. 이러한 부분이 병렬 하드웨어상 속도 개선을 함에 있어 어려운 지점으로 작용한다는 것을 꼭 알고가야합니다. 

{: .prompt-tip}
>최근 아키텍처 분야를 연구하다 현직에서 LLM Optimization을 담당하고 계신 선배님을 만나뵐 기회가 있었습니다. 글을 적다보니 갑자기 선배님이 해주신 “생각보다 많은 이들이 가장 기본적인 원칙을 간과한다. 내가 지금 기본 원칙을 제대로 적용했는지 끊임없이 스스로에게 물어야 한다”라는 조언이 문뜩 떠오르네요. 암달의 법칙을 모르는 사람은 없겠지만, 실제 시스템에서 해당 원칙을 놓친 채 일을 수행하거나 진행되는 연구나 실험은 결국 막대한 시간과 비용을 낭비하기 때문이죠..

### Hardware Parallelism

1960년대 병렬 하드웨어를 분류하는 방법이 하나 제안되었는데, 이것이 오늘날에도 여전히 사용되고 있다고 합니다.
![parallel_hardware_classification_based_on_number_of_command_streams_and_number_of_data_streams](/assets/post/2026-01-17-Hello,-RISC-V!/parallel_hardware_classification_based_on_number_of_command_streams_and_number_of_data_streams.webp)

이 분류법은 `Instruction Stream`의 개수와 `Data Stream`개수에 기반합니다. 눈여겨 봐야 할 것은 `Multieple Data Stream`입니다. 이 두가지가 처음에는 뭐가 다른건지 이해하기 어려운데요? `SIMD`는 x86의 SSE instruction를, `MIMD`로는 Intel Core i7를 생각하시면 대충 어떤 느낌인지 감이 잡히실겁니다.

`SIMD` 컴퓨터는 단일 명령어로 여러 데이터 스트림을 동시에 처리하는` Data-level Parallelism`에 최적화되어 있습니다. 예를 들어, 하나의 `SIMD` 명령어는 64개의 데이터를 64개의 `ALU`로 보내 한 클럭 사이클에 동시에 처리하는게 가능하죠.

`SIMD`는 `Data-level Parallelisim`을 지니는 배열을 처리할 때에 가장 효율적입니다. 그렇기에 `for`문을 처리하는데 있어 굉장히 효율적이죠. 그렇지만 `case`, `switch`문을 처리함에 있어서는 단점을 보입니다. 각 실행 유닛이 자신이 가진 데이터가 무엇인가에 따라 다른 동작을 수행해야 하기 때문이죠. 부적합한 데이터를 가진 실행 유닛들은 동작을 억제시키기 때문에 적합한 데이터를 가진 유닛들만 동작하도록 제어해야하는데, 그러는 과정에서 성능이 급격히 떨어집니다. 

여기서 `SIMD`Instruction을 지원하는 프로세서를 여러개 사용하면 `MIMD`가 됩니다. 그러나 `MIMD`로 되는 순간 신경써야할게 더더 많아지는데요 `MIMD`환경에서는 단순히 계산 속도만 빨라지는 게 아니라 하드웨어 자원을 나눠 쓰는 과정에서 복잡한 숙제가 생깁니다. 이 부분은 책에서도 거의 다루지 않고 저도 아직 잘 모르기때문에 포스팅에서 다루지 않겠습니다.
