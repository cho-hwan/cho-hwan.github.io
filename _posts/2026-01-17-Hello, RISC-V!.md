...
디스크의 경우 전송시간이 접근시간보다 짧기 때문에 쓰기에서는 Write Back 전략을 취하며, `dirty bit`가 사용됩니다. 그리고 TLB가 없으면 메모리 접근은 2번에 거쳐 Sequential하게 진행됩니다. TLB는 메모리 접근을 Parallel하게 가져갈 수 있도록 도와주죠.
...
TLB는 캐시 메커니즘을 따르기에 다양한 세트 연관 방식을 적용할 수 있으나, 일반 캐시 대비 용량이 압도적으로 작다는 구조적 차이가 있습니다. 또한, `Fully Associative`를 도입하면 주소 검색 속도는 향상되지만, LRU와 같은 복잡한 교체 알고리즘을 하드웨어로 구현하는 데 막대한 리소스와 비용이 소모된다는 Trade off가 존재합니다.
...
우선 정석적인 방식 `PIPT(Physically Indexed, Physically Tagged)`입니다. 이 방식은 아무래도 TLB 조회 + Cache Index 조회가 순차적으로(Sequentialy) 수행되어야하므로 다른 방식에 비해 상대적으로 느립니다. 그렇지만 이후에 설명할 VIPT 에서 발생하는 aliasing, homonym 문제가 발생하지 않는다는 장점이 있어 속도보다 용량과 정확성이 중요한 하위레벨 캐시에서 많이 사용됩니다. 실제 루나레이크에서는 L0/L1캐시에서 `VIPT`가, L2/L3 캐시에서는 `PIPT`가 사용되었습니다.
...
가상 메모리는 물리 메모리보다 큰 공간을 활용할 수 있게 해주지만, 프로세스가 실제 메모리 용량보다 과도하게 가상 메모리에 접근하면 디스크 I/O 비용으로 인해 성능이 급격히 저하되는 `Thrashing`현상이 발생합니다. `Thrashing`은 발생 시 손실이 매우 크기 때문에 메모리를 늘리는 것이 일반적인 해결책입니다. 하지만 컴퓨터 전공자라면 알고리즘과 자료구조를 재검토하여 Locality를 개선함으로써 Working Set 규모를 줄이는 시도를 해볼 수 있습니다.
