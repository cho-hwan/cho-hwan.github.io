...
VM은 사실 꽤나 넓은 영역을 지니고있습니다.Java VM같이 표준 소프트웨어 인터페이스를 제공하는 모든 에뮬레이션 또한  VM으로 취급되는데요, 블로그에서 다루는 VM은 ISA수준에서 완전한 시스템 계층 환경을 제공하는 System Virtual Machine이며, IBM VM/730, VirtualBox, Xen들이 그 예입니다.
...
{: .prompt-info}
>요즘은 가상화 기술이 많이 보편화되어 Virtual Box, EC2 와 같은 VM을 다들 많이 접해봤을겁니다. 그리고 VM을 사용하면 완벽한 소프트웨어 스택을 실행할 수 있는 추상화를 제공받을 수 있고, 하드웨어와 무관하게 실행 환경을 어디든 이식할 수 있습니다. 대부분 이러한 장점으로 인해 VM을 사용하죠.

![privilege mode](/assets/post/2026-01-17-Hello,-RISC-V!/privilege_mode.png)

앞서 살펴본 **[RISC-V Privilege Levels]**의 핵심은 가상화 환경에서도 그대로 적용됩니다. VMM은 시스템 전체를 통제해야 하므로, 일반적인 OS나 애플리케이션보다 반드시 높은 권한을 가져야 합니다.
